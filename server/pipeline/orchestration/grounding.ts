...
export async function applyResearchGroundingAndRevalidate(
  ...
  logger.info(
    { letterId, count: citationRegistry.length, isUnverified },
    "[Pipeline] Built citation registry"
  );

  const citationTokens = createTokenAccumulator();
  const researchFromCache =
    opts?.researchFromCache ?? researchProvider === "kv-cache";
  ...
  if (skipRevalidation) {
    ...
    if (citationRegistry.length > 0 && citationRegistry.length < 3)
      reasons.push(`only ${citationRegistry.length} citations (< 3 threshold)`);
    if (isUnverified)
      reasons.push(`research provider ${researchProvider} is not web-grounded`);
    if (researchFromCache)
      reasons.push("research served from KV cache (already validated)");
    if (allHighConfidence)
      reasons.push("all citations already high confidence");
    logger.info(
      { letterId, reasons },
      "[Pipeline] Skipping citation revalidation"
    );
  } else {
    const jurisdiction =
