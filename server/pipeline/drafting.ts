...
export async function runDraftingStage(
    ...
    if (retryFailover && pipelineCtx) {
      ...
      if (!pipelineCtx.qualityWarnings.some(w => w.startsWith("DRAFTING_FAILOVER"))) {
        pipelineCtx.qualityWarnings.push(
          `DRAFTING_FAILOVER: Switched to OpenAI fallback model during draft retry due to rate limit on primary model.`
        );
      }
    }
    return result;
  ...
  try {
    ...
    if (draftFailover) {
      draftLogger.warn({ letterId, provider: draftProvider }, "[Pipeline] Stage 2: Switched to OpenAI fallback model");
      if (pipelineCtx) {
        if (!pipelineCtx.qualityWarnings) pipelineCtx.qualityWarnings = [];
        pipelineCtx.qualityWarnings.push(
          `DRAFTING_FAILOVER: Primary drafting model was rate-limited. Draft generated by OpenAI fallback model.`
        );
      }
    }
