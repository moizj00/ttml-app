...
describe("Auth API Integration Tests", () => {
  ...
  describe("POST /api/auth/google — OAuth initiation and role restriction", () => {
    ...
    it("accepts employee role in OAuth signup and includes role in redirect URL", async () => {
      if (!serverAvailable) return;
      const result = await rawPost("/api/auth/google", {
        intent: "signup",
        role: "employee",
      ...
      expect(result.status).toBe(200);
      expect(result.data.url).toBeTruthy();
      const oauthUrl = String(result.data.url);
      const decoded = decodeURIComponent(oauthUrl);
...
describe("POST /api/auth/google — affiliate role in OAuth URL (local mock server)", () => {
  ...
  it("returns 200 with OAuth URL when role=employee", async () => {
    const result = await localPost("/api/auth/google", { intent: "signup", role: "employee" });
    expect(result.status).toBe(200);
    expect(result.data.url).toBeTruthy();
  });

  it("embeds role=employee in the redirect_to URL parameter (survives round-trip)", async () => {
    const result = await localPost("/api/auth/google", { intent: "signup", role: "employee" });
    expect(result.status).toBe(200);
...
describe("POST /api/auth/google/finalize — affiliate employee role resolution (local mock server)", () => {
  ...
  it("returns redirectPath=/employee when role=employee and no existing user", async () => {
  ...
  it("syncGoogleUser upserts DB with role=employee when requestedRole=employee and no existing user", async () => {
  ...
  it("sends affiliate welcome email via sendEmployeeWelcomeEmail for new affiliate", async () => {
    ...
    // Subscriber welcome email must NOT be sent for an affiliate signup
    expect(mockSendWelcomeEmail).not.toHaveBeenCalled();
  });

  it("returns redirectPath=/dashboard for new subscriber (role=subscriber)", async () => {
  ...
  it("does NOT upgrade returning subscriber to employee via requestedRole (existing role wins)", async () => {
    ...
    mockGetUserByOpenId.mockReset()
      .mockResolvedValueOnce(returningSubscriber)  // existingUser: subscriber
      .mockResolvedValue(returningSubscriber);      // after upsert: still subscriber
    ...
    const result = await localPost("/api/auth/google/finalize", {
      ...
      refresh_token: "valid-refresh-token",
      expires_in: 3600,
      role: "employee",  // requested employee — should be ignored for returning subscriber
    });
    expect(result.status).toBe(200);
    expect(mockUpsertUser).toHaveBeenCalledWith(
  ...
  it("does NOT downgrade returning employee on re-login (existing role wins)", async () => {
    mockGetUserByOpenId.mockReset()
      .mockResolvedValueOnce(newEmployeeUser)  // existingUser: employee
      .mockResolvedValue(newEmployeeUser);     // after upsert: still employee
    mockGetUserById.mockResolvedValue(newEmployeeUser);
    ...
    const result = await localPost("/api/auth/google/finalize", {
      ...
      refresh_token: "valid-refresh-token",
      expires_in: 3600,
      role: "subscriber",  // requested subscriber — should be ignored for returning employee
    });
    expect(result.status).toBe(200);
    expect(mockUpsertUser).toHaveBeenCalledWith(
      expect.objectContaining({ role: "employee" })
  ...
  it("silently rejects attorney requestedRole — new user defaults to subscriber", async () => {
    ...
    const result = await localPost("/api/auth/google/finalize", {
      ...
      refresh_token: "valid-refresh-token",
      expires_in: 3600,
      role: "attorney",  // not self-assignable via OAuth
    });
    expect(result.status).toBe(200);
    expect(mockUpsertUser).toHaveBeenCalledWith(
      expect.objectContaining({ role: "subscriber" })
