...
describe("free preview 24-hour delay UX", () => {
  ...
  it("worker forwards isFreeTrialSubmission into runFullPipeline", () => {
    const source = read("server", "worker.ts");
    expect(source).toContain("usageContext?.isFreeTrialSubmission ?? false");
    expect(source).toMatch(/runFullPipeline\([\s\S]*usageContext\?\.isFreeTrialSubmission \?\? false[\s\S]*\)/);
  });
