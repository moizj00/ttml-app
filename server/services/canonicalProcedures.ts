...
export async function enqueueLetterGenerationProcedure(
  ...
  // Simple pipeline mode: fire-and-forget
  executeLetterGenerationProcedure(requestId, method, pipeline).catch(err => {
    logger.error({ err, requestId }, "Background letter generation failed");
  });
