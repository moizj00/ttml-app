...
describe("updateRole Mutation — Server-Side Guards and Schema", () => {
  ...
  it("updateRole has a subscription conflict guard for attorney promotion", () => {
    // The actual code checks hasActiveRecurringSubscription(input.userId) and throws BAD_REQUEST
    expect(routersFile).toContain("hasActiveRecurringSubscription(input.userId)");
    expect(routersFile).toContain(
      "Cancel their subscription before promoting them to Attorney"
    );
  });

  it("subscription conflict guard message tells admin to cancel subscription first", () => {
    expect(routersFile).toContain(
      "Cancel their subscription before promoting them to Attorney"
    );
  });
