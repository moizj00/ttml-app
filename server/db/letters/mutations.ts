import { and, eq, inArray, isNull, ne, or, sql, notInArray } from "drizzle-orm";
import {
  letterRequests,
  users,
  letterDeliveryLog,
  ...
  if (result.length === 0) {
    ...
    throw new Error(
      `Invalid status transition: ${letter.status} → ${status}. ` +
        `Allowed from ${letter.status}: [${(ALLOWED_TRANSITIONS[letter.status as keyof typeof ALLOWED_TRANSITIONS] ?? []).join(", ")}]`
    );
  }
}
