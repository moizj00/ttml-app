...
describe("admin.updateRole — Active Subscriber Cannot Be Promoted to Attorney", () => {
  ...
  it("throws BAD_REQUEST when target user has an active subscription", async () => {
    // Mock hasActiveRecurringSubscription to return true
    vi.doMock("./stripe", async (importOriginal) => {
      const original = await importOriginal<typeof import("./stripe")>();
      return {
        ...original,
        hasActiveRecurringSubscription: vi.fn().mockResolvedValue(true),
      };
    });
  ...
  it("succeeds when target user has no active subscription", async () => {
    vi.doMock("./stripe", async (importOriginal) => {
      const original = await importOriginal<typeof import("./stripe")>();
      return {
        ...original,
